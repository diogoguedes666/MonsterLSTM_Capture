...
class LossWrapper(nn.Module):
    def __init__(self, losses, pre_filt=None, hf_hinge_fmin=10000, hf_hinge_strength=0.5):
        ...
        # Register loss functions as submodules so they move with .to(device)
        for i, loss_fn in enumerate(self.loss_functions):
            if isinstance(loss_fn, nn.Module):
                self.add_module(f'loss_{i}', loss_fn)
        ...
        # Register loss_factors as buffer so it moves with .to(device)
        if isinstance(self.loss_factors, tuple):
            # Convert tuple to tensor for easier device management
            self.register_buffer('loss_factors_tensor', torch.tensor([f.item() if isinstance(f, torch.Tensor) else f for f in self.loss_factors]))
            self.loss_factors = tuple(self.loss_factors_tensor)

    def forward(self, output, target):
        loss = 0
        for i, losses in enumerate(self.loss_functions):
            # Ensure loss_factors are on the same device as output
            if isinstance(self.loss_factors, tuple):
                factor = self.loss_factors[i].to(output.device) if isinstance(self.loss_factors[i], torch.Tensor) else self.loss_factors[i]
            else:
                factor = self.loss_factors[i] if isinstance(self.loss_factors, tuple) else self.loss_factors
            loss += torch.mul(losses(output, target), factor)
        return loss
